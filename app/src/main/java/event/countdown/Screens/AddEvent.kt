        ...
        AlertDialog(
            ...
            confirmButton = {
                TextButton(onClick = {
                    if (eventName.isNotBlank()) {
                        ...
                        viewModel.onEvent(Eventevent.SetName(eventName))
                        viewModel.onEvent(Eventevent.SetStartTime(startTime.toInt()))
                        viewModel.onEvent(Eventevent.SetEndTim(endTime.toInt()))
                        viewModel.onEvent(Eventevent.SetDate(eventDate))
                        viewModel.onEvent(Eventevent.SaveEvent)
            ...
            text = {
                Column {
                    ...
                    val timeDiff = endTime.toInt() - startTime.toInt()
                    Text(
                        "The Time Is: ${formatTime(timeDiff)}",
                        color = Color(0xFF9C27B0),
                        fontWeight = FontWeight.Bold,
                        modifier = Modifier.padding(top = 16.dp)
                    )
